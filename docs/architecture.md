...
| 11 (classifier) | `planner/query_classifier.py` — deterministic pattern-based query classifier | **Done** — 15/16 (93.8%) accuracy on the eval corpus; escalation rules for ambiguous terms (rule c) and compound questions (rule d) added after concrete routing failures, not speculatively |
...
**Status:** Step 7 (relationship extraction) substantively complete — 7A.1 and 7A.2a done and written; 7A.2b and 7A.3 formally deferred to Phase 3 (code intelligence). **Step 10 (hybrid retrieval) and Step 11 (query planner) are both closed** (§12.15 has the full sub-phase status table). §13 consolidates known graph quality limitations, all either resolved or explicitly scoped as future work — none blocking: the base64/LaTeX chunking artifact from Step 9.5, the 1-hop graph-retrieval depth limit (§12.10, by design), the `organization_fulltext` gap (§12.11, closed), relationship-type-aware graph queries and canonical-aware fusion (§12.12/§12.13, the "connected ≠ relevant" framing Step 11 was built against), the graph-retriever chunk/entity scoring tier (§13 row 9, resolved), and `vector_name`'s full-sentence-query degradation (§12.14, resolved via call-site term extraction in `planner/planner.py`). Step 13 (explainability) scoping is next.
