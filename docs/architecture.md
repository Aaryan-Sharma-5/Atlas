...
| 9 | Within `get_entity_context()`, `evidence_chunk_id`-linked Chunk results and the queried node itself are flatly scored 1.0, same as a genuinely relevant answer-entity at confidence 1.0 — even after relationship-type filtering (§12.12a), chunks can still outrank the actual answer on a high-fan-out node (verified: `paper_2003_02320v6_pdf` filtered to `AUTHORED_BY` puts the target author at rank 14, behind ~12 same-scored chunks) | Retrieval Step 10B/10E (`retrieval/graph_retriever.py`) | **Unmitigated** — chunks and answer-entities currently compete on one scale with no tier separation; not fixed in this pass, flagged for whoever scopes graph-retriever ranking next |

---

**Document Version:** 1.7
**Last Updated:** 2026-07-27
**Status:** Step 7 (relationship extraction) substantively complete — 7A.1 and 7A.2a done and written; 7A.2b and 7A.3 formally deferred to Phase 3 (code intelligence). §13 consolidates known graph quality limitations ahead of Step 10 scoping, including the base64/LaTeX chunking artifact found during Step 9.5 and the 1-hop graph-retrieval depth limit found during Step 10B (§12.10). `organization_fulltext` coverage gap found and closed during Step 10D/10F (§12.11). Relationship-type-aware graph queries and canonical-aware fusion added during Step 10E/10F baseline testing, both instances of the same "connected ≠ relevant" gap (§12.12, §12.13) — this is the framing Step 11 planner scoping starts from.
